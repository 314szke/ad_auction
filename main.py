...
from src.ad_auction_solver import AdAuctionSolver
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Experiment with the ad-auction problem.')
    ...
    parser.add_argument('-n', '--number_of_experiments', type=int, default=20, help='The value of eta will range from 0/n to n/n.')
...
if __name__ == '__main__':
    ...
    for error in args.prediction_error:
        error = include_prediction(data, error, lp_solver.integral_solution)
        solver = AdAuctionSolver(data, verbose=args.verbose)

        best_objective_value = -1
        for k in range(1, args.number_of_experiments + 1):
