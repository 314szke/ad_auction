    ...
    for buyer_id, assigned_items in enumerate(solution):
        for item_id, fraction in assigned_items.items():
            if fraction < 0.0 or fraction > 1.0:
                ...
                valid = False
            budget_spent[buyer_id] += fraction * data.buyers[buyer_id].bids[item_id]
            item_fractions[item_id] += fraction

    for buyer_id, spent in enumerate(budget_spent):
        if int(spent) > int(data.buyers[buyer_id].allowed_budget):
            print(f'ERROR: Buyer {buyer_id} spent {spent} while its allowed budget is {data.buyers[buyer_id].allowed_budget}!')
            valid = False
