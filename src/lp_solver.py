...
class LPSolverWrapper:
    def __init__(self, data, cache_file, verbose):
        ...
        self.bids = [x.bids for x in data.buyers]
    ...
    def _init_model(self, category):
        ...
        for i in self.buyer_ids:
            for j in self.buyers[i].wanted_item_ids:
                weighted_vars.append(self.bids[i][j] * self.vars[i][j])
        self.model += pulp.lpSum(weighted_vars)

        # Constraints
        for i in self.buyer_ids:
            self.model += (pulp.lpSum(self.bids[i][j] * self.vars[i][j] for j in self.buyers[i].wanted_item_ids) <= self.budgets[i], f"budget_{i}")
    ...
    def solve(self):
        ...
        self.model = pulp.LpProblem(name='max-profit-ad-auction', sense=pulp.LpMaximize)
